...
def train_model(net, optimizer, criterion, trainloader, num_ens=1):
    ...
    for i, (inputs, labels) in enumerate(trainloader, 1):
        ...
        loss = criterion(log_outputs, labels.long(), kl)
        loss.backward()
        optimizer.step()
...
def run(dataset, net_type):
    ...
    for epoch in range(n_epochs):  # loop over the dataset multiple times
        ...
        print('Epoch: {} \tTraining Loss: {:.4f} \tTraining Accuracy: {:.4f} '
              ...
              '\ttrain_kl_div: {:.4f}'.format(epoch, train_loss, train_acc, valid_loss, valid_acc, train_kl))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='MNIST', type=str, help='dataset = [MNIST/CIFAR10/CIFAR100/EEG]')
...
# python main_bayesian.py --net_type alexnet --dataset EEG
# python main_bayesian.py --net_type 3conv3fc --dataset EEG
# python main_bayesian.py --net_type lenet --dataset EEG
